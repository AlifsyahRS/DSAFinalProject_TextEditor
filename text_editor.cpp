...
public:
	...
	linked_list(){
		...
		while(choice != 0){
			...
			cout << "Should you want to exit from the program, you may type anything other than the 10 numbers listed above." << endl;
			cout << "Your choice: ";
			cin>>choice;
			cout<<endl;
			...
			{
                double runtime = saveAll();
				cout << "Time to execute: " << runtime << " nanoseconds"<< endl;
	...
	double openFile(){					//function used to open a file from the same folder this cpp file is in
		string fileName;
	...
	void undo(){						//function used to undo the last action taken
		...
		else if (temp.commandNumber == 4) // Corresponds to replacing a text within a line
		{
			cout<<"Replaced line, replacing again..."<<endl;
			replaceTextInLine(temp.text,temp.lineNumber);
			undoStack.pop();
		}
		else if (temp.commandNumber == 5) // Corresponds to inse
		{
			cout<<"Inserted to Head, removing from head..."<<endl;
			deleteHead();
			undoStack.pop();
		}
		else if (temp.commandNumber == 6) // Corresponds to inserting a text that is in-between the head and tail
		{
